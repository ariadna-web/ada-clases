...
const registrarSeries = (nombreSerie, temporadas) => {
    const series = leerSeries();
    series.push({ id: series.length + 2, nombre: nombreSerie, temporadas: temporadas });
...
const actualizarTemporadas = (idSerie, temporadas) => {
    const series = leerSeries();
    let serieEncontrada = series.find((serie) => serie.id == idSerie);
    console.log(serieEncontrada.temporadas)
}
registrarSeries("friends, 10");
// registrarSeries("vikingos, 7");
actualizarTemporadas(2, 6);
listarSeries()